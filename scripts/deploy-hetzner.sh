...
ESCAPED_APP_USERNAME="$(shell_escape "$LOCAL_APP_USERNAME")"
ESCAPED_APP_PASSWORD="$(shell_escape "$LOCAL_APP_PASSWORD")"
ESCAPED_TIME_ZONE="$(shell_escape "$LOCAL_TIME_ZONE")"
ESCAPED_LOCATION_LABEL="$(shell_escape "$LOCAL_LOCATION_LABEL")"
ESCAPED_WEATHER_LATITUDE="$(shell_escape "$LOCAL_WEATHER_LATITUDE")"
ESCAPED_WEATHER_LONGITUDE="$(shell_escape "$LOCAL_WEATHER_LONGITUDE")"
ESCAPED_APP_HOST_PORT="$(shell_escape "$LOCAL_APP_HOST_PORT")"
...
sudo mkdir -p "\$(dirname "${SERVER_ENV_FILE}")"
{
  ...
  echo APP_HOST_PORT=${ESCAPED_APP_HOST_PORT}
  echo FAMILY_HUB_USERNAME=${ESCAPED_APP_USERNAME}
  echo FAMILY_HUB_PASSWORD=${ESCAPED_APP_PASSWORD}
  echo FAMILY_HUB_TIME_ZONE=${ESCAPED_TIME_ZONE}
  echo FAMILY_HUB_LOCATION_LABEL=${ESCAPED_LOCATION_LABEL}
  echo FAMILY_HUB_WEATHER_LATITUDE=${ESCAPED_WEATHER_LATITUDE}
  echo FAMILY_HUB_WEATHER_LONGITUDE=${ESCAPED_WEATHER_LONGITUDE}
  echo "FAMILY_HUB_SEED_DEMO_DATA=0"
